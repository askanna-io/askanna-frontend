...
export function Examples(): Plugin {
    return {
        ...
        async load(id) {
            ...
            const examplesDir = fileURLToPath(new URL('../src/features/style-guide/examples', import.meta.url))
      ...
      `
                ...
                const { imports, files } = (await fs.readdir(path.join(examplesDir, dir), 'utf8'))
                    .reduce<{ imports: string[], files: string[] }>((acc, file, i) => {
                        acc.imports.push(`import __${i} from '/src/features/style-guide/examples/${dir}/${file}'`)
                        acc.imports.push(`import __${i}_raw from '/src/features/style-guide/examples/${dir}/${file}?raw'`)
