...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const getTarget = async () => {
      ...
      const { uuid, short_uuid } = currentChunkData.value
      const registerChunk = await registerChunkPackage({ ...currentChunkData.value })

      const url = `https://api.askanna.eu/v1/package/${short_uuid}/packagechunk/${registerChunk.uuid}/chunk/`
    ...
    const uploadFiles = async () => {
      ...
      fileMetaData.chunks.forEach(async (chunk, index) => {
        ...
        const resultUploadChunk = await chunk.send()
